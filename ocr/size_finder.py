...
def retrieve_all_widths_and_heights(path: str, image_types: List[str]) -> Tuple[List[float], List[float]]:
    ...
    for file in file_list:
        ...
        if osp.splitext(file)[1] in image_types:
            ...
            tmp1, tmp2 = retrieve_all_widths_and_heights(path_to_file, image_types)
            widths.extend(tmp1)
            heights.extend(tmp2)
...
def main() -> None:
    ...
    widths, heights = retrieve_all_widths_and_heights(args.root, config["image_types"])
